...
function replace_macros(macros, text)
    ...
    print("[  \x1b[36mmetamacros.lua\x1b[0m  ]  > \x1b[93m" .. text .. "\x1b[0m -> \x1b[92m" .. new_text .. "\x1b[0m")
...
function processInlines(el)
	...
	if el.t == "Str" and not (s:match("^https?://") or s:match("^#")) and el.text:match("{{(.-)}}") then
		return pandoc.Str(replace_macros(current_macros, el.text))
	else
		return el
	end
end
...
function processEachElement(el)
    if el.t == "Str" then
        ...
        return pandoc.walk_block(el, { Str = processInlines })
    end
end
